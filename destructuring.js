...
/* 기본값 할당 */
const [j1 , js1 , c1 = 'SQL'] = ['java','javascript'];  // 왜 const[j1='java', js1='javascript', c1='SQL']는 오류? 이건 구조분해할당을 위한 형식이니까 '=' 이 있어야 한다. 그냥 배열 선언은 const a = [  ,  ,  ]
console.log(j1);
console.log(js1);
console.log(c1);
console.log("------------------"); 
...
let {userId, userPwd, userName, userAge} = user;     //배열인거랑 아닌거랑 무슨 차이지? 
console.log(userId);
console.log(userPwd);
...
//[요구사항 3] 구조 분해 할당으로 함수 출력해 주세요. 
//방법 1
const person1 = {              //객체 리터럴
     myName : "홍길동",
     age : 25,
     location: "서울시 강남구 테헤란로14길"
};
function example({myName, age, location}){        //왜 이건 중괄호인데? 
     console.log(`${myName} ${age} ${location}`);
}
console.log("함수 호출로 출력");
example(person1);
console.log("--------------------");

//방법 2 함수 exmaple1 에서는 전달된 객체에서 프로퍼티의 value를 매개변수로 대체. 
const example1 = ({myName:n, age:a, location:l}) =>{console.log(`${n} ${a} ${l}`);}  //myName의 value를 n으로 부르겠다. '홍길동'은 똑같음 
example1(person1);
console.log("-------------------")
...
/* 배열의 정렬(sort(): 기본 (문자열 기준) */
const score = [88, 79, 99, 62, 100, 81];
const sortedScore = score.sort((a, b) => a - b); // 오름차순 정렬
//const sortedDesc = score.sort((a, b) => b - a);  // 내림차순 정렬
...
let apple = numbers1.map(myFunction);
                //항목 값, 인덱스, 배열 자체 
function myFunction(value, index, array) { // 이 함수는 3개의 인수를 취한다.
     return  value * 2;                  // 3번 인덱스만 2를 곱하고 싶으면 return (index==3)? value * 2: value;
}
console.log(numbers1.toString()); // 1,2,3,4,5
console.log(apple.toString()); // 2,4,6,8,10

/*const numbers3 = numbers1.map((value, index)=>{
     return value * 5;
});*/

const numbers3 = numbers1.map((value, index) => value * 5);  // 3번 인덱스만 2를 곱하고 싶으면 return (index==3)? value * 2: value;
console.log(numbers3.toString());  // 5,10,15,20,25
console.log("------------------"); 
